		...
		userFilesDropzone: {
			...
			initFileUploader: function (base, target) {
				var previewNode = document.querySelector("#onyx-dropzone-template"), // Dropzone template holder
					warningsHolder = $("#photoMessage"); // Warning messages' holder
				...
				var onyxDropzone = new Dropzone(target, {
							...
							success: function (response) {
								...
								$.each(response, function (key, value) {
									if (typeof value.name !== 'undefined') {
										var mockFile = { name: value.name, upload_ticket: value.name, size: value.size };
					...
					url: ($(target).attr("action")) ? $(target).attr("action") : "action/imageUpdate.php", // Check that our form has an action attr and if not, set one here
					maxFiles: 5,
					maxFilesize: 20, // MB
					acceptedFiles: "image/*",
					 ...
					 */
					dictDefaultMessage: "Drop photos here to upload.<br><br>Only image files types are supported.<br>Maximum file size is 25MB.", // Default: Drop files here to upload
				...
				onyxDropzone.on('addedfile', function () {
					...
					warningsHolder.children('span').html("");
				...
				onyxDropzone.on("success", function (file, response) {
					let parsedResponse = JSON.parse(response);
					file.upload_ticket = parsedResponse.file_name;
