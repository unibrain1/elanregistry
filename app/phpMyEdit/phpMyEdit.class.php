...
/* $Platon: phpMyEdit/phpMyEdit.class.php,v 1.225 2017/11/08 05:27:43 nepto Exp $ */
...
define('PME_FETCH_BOTH',  0);
define('PME_FETCH_ASSOC', 1);
define('PME_FETCH_NUM',   2);
...
class phpMyEdit
{
	...
	function sql_fetch(&$result, $result_type = PME_FETCH_BOTH) /* {{{ */
	{
		if (version_compare(PHP_VERSION, '7.0.0') >= 0) {
			switch($result_type) {
				default:
				case PME_FETCH_BOTH:  $type = MYSQLI_BOTH; break;
				case PME_FETCH_NUM:   $type = MYSQLI_NUM; break;
				case PME_FETCH_ASSOC: $type = MYSQLI_ASSOC; break;
			}
			return mysqli_fetch_array($result, $type);
		} else { // FOR OLD PHP
			switch($result_type) {
				default:
				case PME_FETCH_BOTH:  $type = MYSQL_BOTH; break;
				case PME_FETCH_NUM:   $type = MYSQL_NUM; break;
				case PME_FETCH_ASSOC: $type = MYSQL_ASSOC; break;
			}
	...
	function set_values_from_table($field_num, $strict = false) /* {{{ */
	{
		...
		while ($row = $this->sql_fetch($res, PME_FETCH_NUM)) {
...
function '.$this->js['prefix'].'filter_handler(theForm, theEvent)
{
	...
	function number_of_recs() /* {{{ */
	{
		...
		$row = $this->sql_fetch($res, PME_FETCH_NUM);
