...
/* $Platon: phpMyEdit/extensions/phpMyEdit-report.class.php,v 1.14 2017/06/09 15:02:37 igor Exp $ */
...
class phpMyEdit_report extends phpMyEdit
{
	...
	function execute() /* {{{ */
	{
		...
		$all_table_cols = array_keys($this->sql_fetch($result, FETCH_ASSOC));
		...
		foreach (array_keys($this->fdd) as $field_name) {
			...
			if (($idx = array_search($field_name, $all_table_cols)) !== false)
				$table_cols[$field_name] = $this->sql_field_len($result, $idx);
		}
