...
- [#1538](https://github.com/elan-registry/registry/issues/1538) — fix: pdf-viewer.php wrapper pages flagged as Soft 404 by Google — added per-document titles/descriptions/on-page text and a direct download link, listed the 9 PDFs in `sitemap.xml`, and added a consistency test to keep the two from drifting
- [#1283](https://github.com/elan-registry/registry/issues/1283) — refactor: extracted a shared `TransferIntegrationTestCase` base class, consolidating 4 duplicated transfer-fixture helpers and routing every insert through `CarTransferRepository::create()` instead of raw SQL
- [#1585](https://github.com/elan-registry/registry/issues/1585) — chore: extracted a narrow `ElanRegistry\DatabaseInterface` and a `DbAdapter` wrapper around UserSpice's `\DB`, so production classes can be constructed against a real test double instead of a global mock — deleted the mock `DB`/`QueryResult` classes from `tests/bootstrap-unit.php`, closing #1441's original acceptance criterion. The stricter typing surfaced and fixed several latent production bugs (fatals in `CarRepository::findById()` and `app/owner/contact/owner.php`, a corrupt-backup path in `BackupManager`)
- [#1652](https://github.com/elan-registry/registry/issues/1652) — fix: converted `CarShowcaseService`'s two static methods to constructor-injected instance methods, the one pair #1585's DI migration missed, and added unit coverage including their DB-error fallback paths
...
- [#1679](https://github.com/elan-registry/registry/issues/1679) — tech-debt: converted the install steps that were manual or silently skipped into Phinx migrations — Turnstile hooks, settings baseline defaults (never actually applied on a real install, the root cause of the FilePond `maxFiles: 0` bug), the `noowner` account, and baseline permissions. **Also fixed a silent GDPR-deletion failure:** reassigning cars to `noowner` threw on the deliberately-invalid `noowner@invalid` address, rolling back account deletion and leaving the deleted owner's PII attached to their former cars. Follow-ups: [#1686](https://github.com/elan-registry/registry/issues/1686), [#1547](https://github.com/elan-registry/registry/issues/1547)
...
- [#1702](https://github.com/elan-registry/registry/issues/1702) — fix: `database/4-sample-data.sql` aborted mid-load on a `ModifiedBy` column dropped by an earlier migration, leaving a half-populated database (sample user created, sample car not) with nothing reporting the failure. Fixing the load surfaced that the sample car was never valid registry data — wrong `model` format, wrong `type`, a chassis that fails `ChassisValidator`, and three image filenames that did not exist. Corrected, added a 26R race car for the statistics map's 26R filter test, removed a manual `cars_hist` INSERT the audit trigger already writes, and regenerated the sample password hash to match the documented `password123`
- [#1706](https://github.com/elan-registry/registry/issues/1706) — docs: ADR-006 referenced the deleted `database/3-configuration.sql` in three places. Since ADR-006 is already superseded by ADR-015 and no application code reads the `elan_*_cdn` columns any more, the references are now recorded as history rather than repointed at migrations as if they were live configuration
...
- [#1709](https://github.com/elan-registry/registry/issues/1709) — chore: deleted `app/admin/scripts/fix/_ARCHIVE/` (17 already-executed scripts, three of which would now fatal if run) and rewrote the archive process as a delete process in `FIX_SCRIPTS.md`, the fix-scripts README, and `start-milestone.md`, so no milestone recreates the directory. Also corrected the documented git recovery command, which recovered a stale pre-archive version of every script, and dropped six now-inert `_ARCHIVE/` exclusions from tooling and tests
