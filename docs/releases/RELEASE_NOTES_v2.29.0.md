...
- **#1413 (llms.txt for AI crawler guidance):** After deploying, confirm `curl -s https://elanregistry.org/llms.txt | head` returns AI crawler guidance. On test environment, verify the deploy hook copied `llms-test.txt` over `llms.txt` (mirroring the `robots-test.txt` behavior for test-environment noindex). Watch the next monitoring run for `llms.txt 404` entries to drop to zero.
- **#1414 (bootstrap source maps 404s):** After deploying, confirm `curl -s https://elanregistry.org/users/css/bootstrap.min.css.map | head` and `curl -s https://elanregistry.org/users/js/bootstrap.bundle.min.js.map | head` return source map content (previously 404). The deploy hook auto-vendors these from UserSpice; watch the next monitoring run for Bootstrap CSS/JS source map 404 entries to drop to zero.
