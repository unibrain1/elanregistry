...
- [#1781](https://github.com/elan-registry/registry/issues/1781) — bug: `factory-registry-link.spec.js`/`logged-in.spec.js` self-gated on a `logged-in` Playwright project that never existed locally, so both files silently 0-collected under every local run; added a `setup`+`logged-in` project pair to `playwright.config.js` (auto-authenticates via `TEST_USERNAME`/`TEST_PASSWORD`, skips gracefully if unset), fixed a `testMatch` regex gap that also excluded `factory-registry-link.spec.js` from the deployed `prod`/`test` configs, and fixed six latent bugs the files' first-ever local run exposed (leading-slash `goto()` paths, a DataTables 1.x-only selector, nav locators targeting nonexistent `<nav>`/`<header>` tags, a hardcoded-prod URL in a link checker, and a missing fixture-gap skip) — both files now run and pass locally
