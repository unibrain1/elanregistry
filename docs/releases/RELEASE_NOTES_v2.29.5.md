...
- **Database errors on owner/car lookups no longer fail silently** ([#1505](https://github.com/elan-registry/registry/issues/1505), [#1656](https://github.com/elan-registry/registry/issues/1656)): `Owner`, `CarRepository`, and `CarModel` now throw a typed exception on a DB failure instead of returning `false`/`null`/`[]` indistinguishable from "not found." `Owner::create()`/`update()` also gained checked transaction handling, so a mid-write PHP error reliably rolls back instead of possibly leaving a half-written record.
